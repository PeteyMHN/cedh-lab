...
export class Game {
  ...
  constructor(playerConfigs: PlayerConfig[], opts: { seed: number | string; startingLife?: number; freeMulligans?: number } = { seed: 1 }) {
    ...
    this.freeMulligans = opts.freeMulligans ?? 0;
    playerConfigs.forEach((c, i) => {
      ...
      this.players.push({
        ...
        manaPool: emptyMana(), commanderIds: commanders.map((o) => o.id),
  ...
  moveZone(objId: string, to: Zone, toPlayer?: number): void {
    ...
    const from = o.zone;
    if (from === 'stack') {
      const i = this.turn.stack.findIndex((s) => s.id === objId || s.sourceId === objId);
  ...
  draw(p: number, n = 1): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.emit('DRAW', { player: p, object: id, card: this.getObject(id).cardName });
    }
  }

  /** Partial Paris mulligan (cEDH multiplayer): draw 7-n, then bottom n. */
  mulligan(p: number): void {
    const pl = this.players[p];
    if (pl.mulligansTaken >= 7) throw new Error('no more mulligans');
    const n = pl.mulligansTaken + 1;
    // return hand, shuffle, draw 7, bottom n (free mulligans don't bottom)
    ...
    const toBottom = n - free;
    for (let i = 0; i < toBottom && pl.hand.length > 0; i++) {
      const id = pl.hand.pop()!;
      pl.library.push(id);
      this.getObject(id).zone = 'library';
    }
    this.emit('MULLIGAN', { player: p, count: n, bottomed: toBottom, free });
  }
